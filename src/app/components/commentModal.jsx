...
export default function CommentModel({ post_id }) {
  ...
  const fetchCommentsByPost = async () => {
    try {
      ...
      if (data.success) {
        console.log("Comments for this post:", data.data);
        setComments(data.data);
      } else {
        console.warn("No comments found or error:", data.message);
      }
    } catch (error) {
      console.error("API Error:", error);
    }
  ...
  // 🟢 Add a new comment (frontend + backend)
  const handleAddComment = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/comments", {
        ...
        body: JSON.stringify({
          user_id: "your-user-id-here", // 🔹 replace with logged-in user_id
          post_id,
          comment_text: newComment,
        }),
      });
